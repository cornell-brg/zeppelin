...
task test_case_directed_remu_2_x0();
  ...
  h.asm( 'h208, "remu x0, x0, x1" );
  h.asm( 'h20c, "remu x3, x0, x2" );

  // Check each executed instruction

  h.check_trace( 'h200,  1, 'h01, 1 ); // addi x1, x0, 1
  h.check_trace( 'h204,  2, 'h02, 1 ); // addi x2, x0, 2
  h.check_trace( 'h208, 'x, 'x,   0 ); // remu x0, x0, x1
  h.check_trace( 'h20c,  3, 'h00, 1 ); // remu x3, x0, x2
...
task test_case_directed_remu_3_pos();
  ...
  h.asm( 'h210, "remu x5,  x1,  x2"   );
  h.asm( 'h214, "remu x6,  x2,  x3"   );
  h.asm( 'h218, "remu x7,  x3,  x4"   );

  // Check each executed instruction

  h.check_trace( 'h200, 1, 24,        1 ); // addi x1,  x0,  24
  h.check_trace( 'h204, 2, 12,        1 ); // addi x2,  x0,  12
  h.check_trace( 'h208, 3, 5,         1 ); // addi x3,  x0,  3
  h.check_trace( 'h20c, 4, 2,         1 ); // addi x4,  x0,  2

  h.check_trace( 'h210, 5, 0,         1 ); // remu x5,  x1,  x2
  h.check_trace( 'h214, 6, 2,         1 ); // remu x6,  x2,  x3
  h.check_trace( 'h218, 7, 1,         1 ); // remu x7,  x3,  x4
...
task test_case_directed_remu_4_neg();
  ...
  h.check_trace( 'h210, 5, 4,           1 ); // remu x5,  x1,  x2
  h.check_trace( 'h214, 6, 12,          1 ); // remu x6,  x2,  x3
  h.check_trace( 'h218, 7, 'hffff_fffa, 1 ); // remu x7,  x3,  x4
...
task test_case_directed_remu_5_zero();
  ...
  h.asm( 'h204, "remu x2,  x1,  x0"   );
  h.asm( 'h208, "remu x3,  x0,  x0"   );

  // Check each executed instruction

  h.check_trace( 'h200, 1, 'h0000_0064, 1 ); // addi x1,  x0,  0x64
  h.check_trace( 'h204, 2, 'h0000_0064, 1 ); // remu x2,  x1,  x0
  h.check_trace( 'h208, 3, 'h0000_0000, 1 ); // remu x3,  x0,  x0
...
task test_case_directed_remu_6_overflow();
  ...
  h.check_trace( 'h208, 3, 'h8000_0000, 1 ); // remu x3,  x1,  x2
