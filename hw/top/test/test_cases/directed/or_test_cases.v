...
task test_case_directed_or_2_x0();
  ...
  h.asm( 'h208, "or   x0, x0, x0" );
  h.asm( 'h20c, "or   x0, x0, x1" );
  h.asm( 'h210, "or   x0, x2, x0" );
  h.asm( 'h214, "or   x3, x0, x1" );
  h.asm( 'h218, "or   x4, x2, x0" );

  // Check each executed instruction

  h.check_trace( 'h200,  1, 'h01, 1 ); // addi x1, x0, 1
  h.check_trace( 'h204,  2, 'h02, 1 ); // addi x2, x0, 2
  h.check_trace( 'h208, 'x, 'x,   0 ); // or   x0, x0, x0
  h.check_trace( 'h20c, 'x, 'x,   0 ); // or   x0, x0, x1
  h.check_trace( 'h210, 'x, 'x,   0 ); // or   x0, x2, x0
  h.check_trace( 'h214,  3, 'h01, 1 ); // or   x3, x0, x1
  h.check_trace( 'h218,  4, 'h02, 1 ); // or   x4, x2, x0
...
task test_case_directed_or_3_extreme();
  ...
  h.asm( 'h20c, "or   x4,  x1,  x2"   );
  h.asm( 'h210, "or   x5,  x2,  x3"   );
  h.asm( 'h214, "or   x6,  x3,  x1"   );
  h.asm( 'h218, "or   x7,  x1,  x0"   );
  h.asm( 'h21c, "or   x8,  x1,  x1"   );
  h.asm( 'h220, "or   x9,  x0,  x0"   );
  ...
  h.check_trace( 'h20c, 4, 'hffff_ffff,    1 ); // or   x4,  x1,  x2
  h.check_trace( 'h210, 5, 'h0000_07ff,    1 ); // or   x5,  x2,  x3
  h.check_trace( 'h214, 6, 'hffff_ffff,    1 ); // or   x6,  x3,  x1
  h.check_trace( 'h218, 7, 'hffff_ffff,    1 ); // or   x7,  x1,  x0
  h.check_trace( 'h21c, 8, 'hffff_ffff,    1 ); // or   x8,  x1,  x1
  h.check_trace( 'h220, 9, 'h0000_0000,    1 ); // or   x9,  x0,  x0
