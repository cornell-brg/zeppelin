...
//------------------------------------------------------------------------
// test_case_directed_bge_1_basic
//------------------------------------------------------------------------

task test_case_directed_bge_1_basic();
  h.t.test_case_begin( "test_case_directed_bge_1_basic" );
  ...
  h.check_trace( 'h204, 'x, 'x,          0 ); // bge  x1, x0, 0x20c
...
//------------------------------------------------------------------------
// test_case_directed_bge_2_taken
//------------------------------------------------------------------------

task test_case_directed_bge_2_taken();
  h.t.test_case_begin( "test_case_directed_bge_2_taken" );
  if( !h.t.run_test ) return;
  fl_reset();
...
//------------------------------------------------------------------------
// test_case_directed_bge_3_not_taken
//------------------------------------------------------------------------

task test_case_directed_bge_3_not_taken();
  h.t.test_case_begin( "test_case_directed_bge_3_not_taken" );
  if( !h.t.run_test ) return;
  fl_reset();
...
//------------------------------------------------------------------------
// test_case_directed_bge_4_chain
//------------------------------------------------------------------------

task test_case_directed_bge_4_chain();
  h.t.test_case_begin( "test_case_directed_bge_4_chain" );
  if( !h.t.run_test ) return;
  fl_reset();
  ...
  h.asm( 'h218, "bge  x2,  x1, 0x220" );
  h.asm( 'h21c, "addi x7,  x0, 2"     );
  h.asm( 'h220, "bge  x4,  x3, 0x228" );
  h.asm( 'h224, "addi x8,  x0, 3"     );
  h.asm( 'h228, "bge  x6,  x5, 0x230" );
  ...
  h.check_trace( 'h218, 'x, 'x, 0 ); // bge  x2,  x1, 0x220
  h.check_trace( 'h220, 'x, 'x, 0 ); // bge  x4,  x3, 0x228
  h.check_trace( 'h228, 'x, 'x, 0 ); // bge  x6,  x5, 0x230
...
//------------------------------------------------------------------------
// test_case_directed_bge_5_backward
//------------------------------------------------------------------------

task test_case_directed_bge_5_backward();
  h.t.test_case_begin( "test_case_directed_bge_5_backward" );
  if( !h.t.run_test ) return;
  fl_reset();
  ...
  h.check_trace( 'h210, 'x, 'x, 0 ); // bge   x1,  x0, 0x244
  h.check_trace( 'h244, 14, 11, 1 ); // addi x14,  x0, 11
  h.check_trace( 'h248, 'x, 'x, 0 ); // bge   x0, x15, 0x238
  h.check_trace( 'h238, 11,  9, 1 ); // addi x11,  x0, 9
  h.check_trace( 'h23c, 'x, 'x, 0 ); // bge   x0, x12, 0x228
  h.check_trace( 'h228,  7,  6, 1 ); // addi  x7,  x0, 6
  h.check_trace( 'h22c, 'x, 'x, 0 ); // bge   x8,  x0, 0x214
...
//------------------------------------------------------------------------
// test_case_directed_bge_6_loop
//------------------------------------------------------------------------

task test_case_directed_bge_6_loop();
  h.t.test_case_begin( "test_case_directed_bge_6_loop" );
  if( !h.t.run_test ) return;
  fl_reset();

  // Write assembly program into memory

  h.asm( 'h200, "addi x1, x0, 3"    ); //
  h.asm( 'h204, "addi x1, x1, -1"    ); // <-.
  h.asm( 'h208, "addi x1, x1, -1"    ); //   |
  h.asm( 'h20c, "bge  x1, x0, 0x204" ); // --'
  h.asm( 'h210, "addi x2, x0, 1"     ); //
  h.asm( 'h214, "addi x3, x0, 2"     ); //

  // Check each executed instruction

  h.check_trace( 'h200,  1,  3, 1 ); // addi x1, x0, -3
  h.check_trace( 'h204,  1,  2, 1 ); // addi x1, x1, 1
  h.check_trace( 'h208,  1,  1, 1 ); // addi x1, x1, 1
  h.check_trace( 'h20c, 'x, 'x, 0 ); // bge  x1, x0, 0x204
  h.check_trace( 'h204,  1,  0, 1 ); // addi x1, x1, -1
  h.check_trace( 'h208,  1, -1, 1 ); // addi x1, x1, -1
  h.check_trace( 'h20c, 'x, 'x, 0 ); // bge  x1, x0, 0x204
  h.check_trace( 'h210,  2,  1, 1 ); // addi x2, x0, 1
  h.check_trace( 'h214,  3,  2, 1 ); // addi x3, x0, 2

  h.t.test_case_end();
endtask

//------------------------------------------------------------------------
// test_case_directed_bge_7_loop_self
//------------------------------------------------------------------------

task test_case_directed_bge_7_loop_self();
  h.t.test_case_begin( "test_case_directed_bge_7_loop_self" );
  if( !h.t.run_test ) return;
  fl_reset();
  ...
  h.asm( 'h200, "addi x1, x0, 1"    );
  h.asm( 'h204, "bge  x1, x0, 0x204" );

  // Check each executed instruction

  h.check_trace( 'h200,  1,  1, 1 ); // addi x1, x0, 1
  h.check_trace( 'h204, 'x, 'x, 0 ); // bge  x1, x0, 0x204
  h.check_trace( 'h204, 'x, 'x, 0 ); // bge  x1, x0, 0x204
  h.check_trace( 'h204, 'x, 'x, 0 ); // bge  x1, x0, 0x204
  h.check_trace( 'h204, 'x, 'x, 0 ); // bge  x1, x0, 0x204
