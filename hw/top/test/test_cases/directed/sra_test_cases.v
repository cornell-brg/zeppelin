...
task test_case_directed_sra_2_x0();
  ...
  h.asm( 'h208, "sra  x0, x0, x0" );
  h.asm( 'h20c, "sra  x0, x0, x1" );
  h.asm( 'h210, "sra  x0, x2, x0" );
  h.asm( 'h214, "sra  x3, x0, x1" );
  h.asm( 'h218, "sra  x4, x2, x0" );

  // Check each executed instruction

  h.check_trace( 'h200,  1, 'h01, 1 ); // addi x1, x0, 1
  h.check_trace( 'h204,  2, 'h02, 1 ); // addi x2, x0, 2
  h.check_trace( 'h208, 'x, 'x,   0 ); // sra  x0, x0, x0
  h.check_trace( 'h20c, 'x, 'x,   0 ); // sra  x0, x0, x1
  h.check_trace( 'h210, 'x, 'x,   0 ); // sra  x0, x2, x0
  h.check_trace( 'h214,  3, 'h00, 1 ); // sra  x3, x0, x1
  h.check_trace( 'h218,  4, 'h02, 1 ); // sra  x4, x2, x0
...
task test_case_directed_sra_3_upper();
  ...
  h.asm( 'h210, "sra  x5,  x3,  x1"   );
  h.asm( 'h214, "sra  x6,  x3,  x2"   );
  h.asm( 'h218, "sra  x7,  x3,  x3"   );
  h.asm( 'h21c, "sra  x8,  x1,  x1"   );
  h.asm( 'h220, "sra  x9,  x3,  x4"   );
  h.asm( 'h224, "sra  x10, x2,  x4"   );
  ...
  h.check_trace( 'h210, 5,  'h0000_0000, 1 ); // sra  x5,  x3,  x1
  h.check_trace( 'h214, 6,  'h0000_000a, 1 ); // sra  x6,  x3,  x2
  h.check_trace( 'h218, 7,  'h0000_0000, 1 ); // sra  x7,  x3,  x3
  h.check_trace( 'h21c, 8,  'hffff_ffff, 1 ); // sra  x8,  x1,  x1
  h.check_trace( 'h220, 9,  'h0000_0002, 1 ); // sra  x9,  x3,  x4
  h.check_trace( 'h224, 10, 'hffff_feb8, 1 ); // sra  x10, x2,  x4
