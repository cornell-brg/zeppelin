...
void dotrav( void )
{
  ...
  if ( !mvflag )
    ...
    newloc = rdest;
    if ( dbugflg )
      zeppelin_printf( "newloc in dotrav = %d\n", newloc );
  }
}
...
void doobj( void )
{
  ...
  */
    ...
    zeppelin_printf( "I see no %s here.\n", probj( object ) );
}
...
void trobj( void )
{
  if ( verb )
    ...
    zeppelin_printf( "What do you want to do with the %s?\n",
                  probj( object ) );
}
...
void dwarves( void )
{
  ...
  */
  ...
  for ( i = 1; i < DWARFMAX; ++i ) {
    ...
    */
    for ( try = 1; try < 20; ++try ) {
      j = zeppelin_rand() % 106 + 15; /* allowed area */
      if ( j != odloc[i] && j != dloc[i] &&
           !( i == ( DWARFMAX - 1 ) &&
              ( cond[j] & NOPIRAT ) == NOPIRAT ) )
        break;
    }
    if ( j == 0 )
      j = odloc[i];
    odloc[i] = dloc[i];
    dloc[i]  = j;
    if ( ( dseen[i] && newloc >= 15 ) || dloc[i] == newloc ||
         odloc[i] == newloc )
      dseen[i] = 1;
    else
      dseen[i] = 0;
    ...
    if ( i == 6 )
      ...
      if ( odloc[i] == dloc[i] ) {
        ...
        if ( knfloc >= 0 )
          knfloc = newloc;
        if ( zeppelin_rand() % 1000 < 95 * ( dflag - 2 ) )
          ++stick;
      }
    }
  }
  ...
  if ( dflag == 2 )
    ++dflag;
  if ( attack > 1 ) {
    zeppelin_printf( "%d of them throw knives at you!!\n", attack );
    k = 6;
  }
  else {
    rspeak( 5 );
  ...
  if ( stick <= 1 ) {
    ...
    zeppelin_printf( "%d of them get you !!!\n", stick );
  oldloc2 = newloc;
  death();
}
/*
