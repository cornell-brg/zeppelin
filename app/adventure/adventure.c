...
#define setmem( l, s, c ) zeppelin_memset( l, c, s )
...
int main( int argc, char *argv[] )
{
  ...
  while ( --argc > 0 ) {
    ...
    if ( **argv != '-' )
      break;
    switch ( zeppelin_tolower( argv[0][1] ) ) {
      case 'r':
        ++rflag;
        continue;
      case 'd':
        ...
        continue;
      default:
        zeppelin_printf( "unknown flag: %c\n", argv[0][1] );
        continue;
    }
  }
  ...
  if ( rflag )
    ...
    limit = 330;
  saveflg = 0;
  zeppelin_srand( 511 ); /* seed random	*/
  while ( !saveflg )
    turn();
  if ( saveflg )
    saveadv();
...
void opentxt( void )
{
  ...
  fd1 = fopen( ADV1, "r" );
  if ( !fd1 ) {
    zeppelin_printf( "Sorry, I cannot open %s...\n", ADV1 );
    exit( -1 );
  }
  fd2 = fopen( ADV2, "r" );
  if ( !fd2 ) {
...
{
#ifdef _RISCV
  zeppelin_printf( "Sorry, saving the game for RISCV isn't implemented\n" );
#else
  char  nm[64];
  FILE *fp;
  ...
  fp = fopen( nm, "w" );
  if ( !fp ) {
    zeppelin_printf( "Sorry, I cannot save your game to %s", nm );
  }
  ...
  fwrite( &visited[0], sizeof( int ), MAXLOC,
          fp );                                  /* >0 if has been here	*/
  ...
  fwrite( &odloc[0], sizeof( int ), DWARFMAX,
          fp );                             /* dwarf old locations	*/
  ...
  if ( fclose( fp ) == -1 ) {
    zeppelin_printf( "Sorry, I cannot seem to close the save game file %s",
                  nm );
  }

  zeppelin_printf( "Game saved to %s -- see you later!\n", nm );
#endif  // _RISCV
}
...
void restore( void )
{
  ...
  fp = fopen( nm, "r" );
  if ( fp == NULL ) {
    zeppelin_printf( "Sorry, cannot find any saved game to load from %s",
                  nm );
    return;
  }
  ...
  if ( fread( &turns, sizeof( int ), 1, fp ) != 1 ||
       ...
       fread( &newloc, sizeof( int ), 1, fp ) !=
           1 || /* location variables	*/
       fread( &cond[0], sizeof( int ), MAXLOC, fp ) !=
           MAXLOC || /* location status	*/
       fread( &place[0], sizeof( int ), MAXOBJ, fp ) !=
           MAXOBJ || /* object location	*/
       fread( &fixed[0], sizeof( int ), MAXOBJ, fp ) !=
           MAXOBJ || /* second object loc	*/
       fread( &visited[0], sizeof( int ), MAXLOC, fp ) !=
           MAXLOC || /* >0 if has been here	*/
       fread( &prop[0], sizeof( int ), MAXOBJ, fp ) !=
           MAXOBJ || /* status of object	*/
       fread( &tally, sizeof( int ), 1, fp ) != 1 ||
       fread( &tally2, sizeof( int ), 1, fp ) !=
           1 || /* item counts		*/
       fread( &limit, sizeof( int ), 1, fp ) !=
           1 || /* time limit		*/
       fread( &lmwarn, sizeof( int ), 1, fp ) !=
           1 || /* lamp warning flag	*/
       fread( &wzdark, sizeof( int ), 1, fp ) != 1 ||
       fread( &closing, sizeof( int ), 1, fp ) != 1 ||
       fread( &closed, sizeof( int ), 1, fp ) !=
           1 || /* game state flags	*/
       fread( &holding, sizeof( int ), 1, fp ) !=
           1 || /* count of held items	*/
       fread( &detail, sizeof( int ), 1, fp ) !=
           1 ||                                       /* LOOK count		*/
       fread( &knfloc, sizeof( int ), 1, fp ) != 1 || /* knife location	*/
       fread( &clock1, sizeof( int ), 1, fp ) != 1 ||
       fread( &clock2, sizeof( int ), 1, fp ) != 1 ||
       fread( &panic, sizeof( int ), 1, fp ) !=
           1 || /* timing variables	*/
       fread( &dloc[0], sizeof( int ), DWARFMAX, fp ) !=
           DWARFMAX || /* dwarf locations	*/
       fread( &dflag, sizeof( int ), 1, fp ) !=
           1 || /* dwarf flag		*/
       fread( &dseen[0], sizeof( int ), DWARFMAX, fp ) !=
           DWARFMAX || /* dwarf seen flag	*/
       fread( &odloc[0], sizeof( int ), DWARFMAX, fp ) !=
           DWARFMAX || /* dwarf old locations	*/
       fread( &daltloc, sizeof( int ), 1, fp ) !=
           1 || /* alternate appearance	*/
       fread( &dkill, sizeof( int ), 1, fp ) != 1 || /* dwarves killed	*/
       fread( &chloc, sizeof( int ), 1, fp ) != 1 ||
       fread( &chloc2, sizeof( int ), 1, fp ) !=
           1 ||                                      /* chest locations	*/
       fread( &bonus, sizeof( int ), 1, fp ) != 1 || /* to pass to end	*/
       fread( &numdie, sizeof( int ), 1, fp ) !=
           1 || /* number of deaths	*/
       fread( &object1, sizeof( int ), 1, fp ) !=
           1 || /* to help intrans.	*/
       fread( &gaveup, sizeof( int ), 1, fp ) !=
           1 || /* 1 if he quit early	*/
       fread( &foobar, sizeof( int ), 1, fp ) !=
           1 ) { /* fie fie foe foo...	*/
    zeppelin_printf( "Failed reading saved gave file, %s, data format error",
                  nm );
    fclose( fp );
    return;
  }
  ...
  zeppelin_printf( "Game restored from %s\n", nm );
  describe();
#endif  // _RISCV
}
