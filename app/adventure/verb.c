...
void trverb( void )
{
  switch ( verb ) {
    ...
    case WAKE:
      ...
      break;
    default:
      zeppelin_printf( "This verb is not implemented yet.\n" );
  }
}
...
void vwave( void )
{
  if ( !toting( object ) && ( object != ROD || !toting( ROD2 ) ) )
    rspeak( 29 );
  else if ( object != ROD || !at( FISSURE ) || !toting( object ) ||
            closing )
    actspk( verb );
  else {
...
void vfind( void )
{
  ...
  if ( toting( object ) )
    ...
    msg = 94;
  else if ( at( object ) || ( liq() == object && here( BOTTLE ) ) ||
            object == liqloc( loc ) )
    msg = 94;
  else {
    actspk( verb );
